...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоНовый ИЛИ (Параметры.Свойство("ЗаполнитьКакНовый") И Параметры.ЗаполнитьКакНовый) Тогда
		...
		//**************************
		Объект.ВидДокумента=Справочники.аДокументооборотВидыДокументов.НайтиПоНаименованию("Заявка на оплату");
		ВидДокумента=Объект.ВидДокумента;
		//**************************
	...
	Иначе //не новый
		...
		Если ПустаяСтрока(Объект.СодержаниеДокумента) И Объект.СсылкиНаФайлы.Количество()>0 Тогда
			Элементы.Страницы1.ТекущаяСтраница=Элементы.Страницы1.ПодчиненныеЭлементы.Файлы;
	...
	Элементы.СписокПолучателей.Видимость=Объект.ПрисутствуютПолучатели;
	...
	КонецЕсли;
	//***************************************************************************************
	pcru_БизнесПроцессы.ПолучитьПользователейДокумента(ЭтотОбъект.Объект);
	
	УстиновитьЗначениеРеквизитов();
	
	УстановитьПараметровСписков();
	
	//Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
	//	Если  ЭтотОбъект.Объект.КонтрагентыДоговоры.Количество() > 0 Тогда
	//		//Если ЭтотОбъект.Объект.КонтрагентыДоговоры[0].Контрагент.Наименование = "Комус-Петербург ООО" Тогда
	//		//	СписокЗадачПоДокументу.Параметры.УстановитьЗначениеПараметра("ЕстьКоординатор", Истина);
	//		//иначе
	//		//	СписокЗадачПоДокументу.Параметры.УстановитьЗначениеПараметра("ЕстьКоординатор", Ложь);
	//		//КонецЕсли; 
	//	Иначе 
	//	   СписокЗадачПоДокументу.Параметры.УстановитьЗначениеПараметра("ЕстьКоординатор", Ложь);
	//	КонецЕсли; 
	//	СписокЗадачПоДокументу.Параметры.УстановитьЗначениеПараметра("Сумма", ЭтотОбъект.Объект.СуммаДокумента);
	//	Если ЭтотОбъект.Объект.ОрганизацииПодразделения.Количество() > 0 Тогда
	//			СписокЗадачПоДокументу.Параметры.УстановитьЗначениеПараметра("Отдел", ЭтотОбъект.Объект.ОрганизацииПодразделения[0].Подразделение.Код);
	//	КонецЕсли; 
	//Иначе
	//     СписокЗадачПоДокументу.Параметры.УстановитьЗначениеПараметра("ЕстьКоординатор", Ложь);
	//	СписокЗадачПоДокументу.Параметры.УстановитьЗначениеПараметра("Сумма", ЭтотОбъект.Объект.СуммаДокумента);
	//	СписокЗадачПоДокументу.Параметры.УстановитьЗначениеПараметра("Отдел","CEO");
	//КонецЕсли;
	
	НеПолныеПрава  = не аДООбщееСерверПС.ЭтоРольПолныеПрава();
	
	Если НеПолныеПрава Тогда
		НеПолныеПрава = ИмяПользователя() <> "iefimova";
	КонецЕсли;
	
	Если НеПолныеПрава Тогда
		НеПолныеПрава = ИмяПользователя() <> "mchumakova";
	КонецЕсли;
	
	Если НеПолныеПрава Тогда
		НеПолныеПрава = ИмяПользователя() <> "itimoshenkova";
	КонецЕсли;
	
	Элементы.ФормаОтправитьВУМФО.Доступность = Ложь;
	ТекущееСостояниеДокумента=аДОРаботаСДокументами.ПрочитатьСостояниеДокумента(Объект.Ссылка);
	Если ЗначениеЗаполнено(ТекущееСостояниеДокумента)  Тогда
	  Если ТекущееСостояниеДокумента.Наименование = "В оплате" Тогда
		Элементы.ФормаОтправитьВУМФО.Доступность = Истина;
	  КонецЕсли; 
	КонецЕсли; 
	
	Если НеПолныеПрава Тогда
		Если не ЭтоНовый и pcru_БизнесПроцессы.ЕстьСтартованныеБП(ЭтотОбъект.Объект.Ссылка) Тогда
			ЭтаФорма.ТолькоПросмотр = Истина;
			Элементы.ФормаПрерватьСогласовоние.Доступность = Истина;
		Иначе
			Элементы.ФормаПрерватьСогласовоние.Доступность = Ложь; 
		КонецЕсли; 
		Элементы.ФормаОтправитьВУМФО.Доступность = Ложь;
		...
		БулевоРСИ  = Объект.СписокПолучателей.Количество() > 0;
		
		ЭтаФорма.РучнойСписокИсполнителей = БулевоРСИ;
		Элементы.ГруппаСписокИсполнителей.Видимость	= ЭтаФорма.РучнойСписокИсполнителей;
...
Процедура УстановитьПараметровСписков()
	...
	СписокВозможныхИсполнителей.Порядок.Элементы.Очистить();
...
Процедура УстановитьЗапретРедактированияФормы(Установить=Истина)
	...
	Если Установить Тогда
		...
		Если РазрешитьИзменениеСпискаПолучателейПодписанногоДокумента Тогда
			...
			Элементы.СписокПолучателей.ТолькоПросмотр=Ложь;
			...
			Для Каждого ЭлементРегистрации Из Элементы.СписокПолучателей.ПодчиненныеЭлементы Цикл
				//разрешим доступ
...
Процедура СогласоватьНаСервере(УжеСогласован)
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.СостояниеДокумента = Справочники.аДокументооборотСостояниеДокументов.Согласован или Выборка.СостояниеДокумента = Справочники.аДокументооборотСостояниеДокументов.НайтиПоНаименованию("Оплачен") Тогда
			Сообщить("Этот документ уже оплачен");
			УжеСогласован = Истина;
			Возврат;
		КонецЕсли; 
	КонецЦикла;
...
Процедура Согласовать(Команда)
	Для каждого Стр  Из Объект.пкру_РаспределениеЗатрат Цикл
		Если не ЗначениеЗаполнено(Стр.пкру_CostCentr)  Тогда
			ПоказатьПредупреждение(,"Заполните все поля CostCentr !!",10,"Не возможно согласовать!");	
			Возврат;
		КонецЕсли; 
		Если не ЗначениеЗаполнено(Стр.пкру_СтатьяРасходов)  Тогда
			ПоказатьПредупреждение(,"Заполните все поля Статья Расходов !!",10,"Не возможно согласовать!");	
			Возврат;
		КонецЕсли; 
	КонецЦикла; 
	Если Объект.СуммаДокумента = 0 Тогда
		ПоказатьПредупреждение(,"Сумма не должна быть 0 !!",10,"Не возможно согласовать!");	
		Возврат;
	КонецЕсли; 
	УжеСогласован = Ложь;
...
&НаСервере
Процедура УстиновитьЗначениеРеквизитов()
	
	Если Объект.КонтрагентыДоговоры.Количество() > 0 Тогда
		ЭтотОбъект.pcru_Контрагент	    = Объект.КонтрагентыДоговоры[0].Контрагент;
		ЭтотОбъект.pcru_договорКонтрагента= Объект.КонтрагентыДоговоры[0].ДоговорКонтрагента;
	КонецЕсли;
	...
	РеквизитФормыУстановить("Это Тендер",ЭтаФорма.pcru_ЭтоТендер);
	РеквизитФормыУстановить("Окончательный Счет",ЭтаФорма.pcru_ОкончательныйСчет);
	РеквизитФормыУстановить("Предоплата",ЭтаФорма.pcru_Предоплата);
	РеквизитФормыУстановить("Частичный платеж",ЭтаФорма.pcru_ЧастичныйПлатеж);
	РеквизитФормыУстановить("Описание Тендера",ЭтаФорма.pcru_ОписаниеТендера);
	
	
КонецПроцедуры

&НаСервере
Процедура ПечатьЛистаСогласованияНаСервере(ТД)
	pcru_БизнесПроцессы.Бит_ПечатьЛистаСогласования(Объект.Ссылка,ТД);
КонецПроцедуры

&НаКлиенте
Процедура ПечатьЛистаСогласования(Команда)
	ТД = Неопределено;
	ПечатьЛистаСогласованияНаСервере(ТД);
	ТД.Показать();
КонецПроцедуры

&НаКлиенте
Процедура pcru_договорКонтрагентаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	// Вставить содержимое обработчика.
КонецПроцедуры

&НаСервере
Процедура РучнойСписокИсполнителейПриИзмененииНаСервере()
	Элементы.ГруппаСписокИсполнителей.Видимость	= ЭтаФорма.РучнойСписокИсполнителей; // Вставить содержимое обработчика.
КонецПроцедуры

&НаКлиенте
Процедура РучнойСписокИсполнителейПриИзменении(Элемент)
	РучнойСписокИсполнителейПриИзмененииНаСервере();
КонецПроцедуры
...
&НаСервере
функция пкру_РаспределениеЗатратпкру_СтатьяРасходовПриИзмененииНаСервере(CostCentr,СтатьяРасходов)
	//	  Операции	          Тип затрат
	//по кост-центру 2.05.ХХ	2
	//по кост-центту 2.08.00	2
	//По кост-центру 2.11.01	2
	//По статье расходов 1395 за исключением кост-центров 2.07.ХХ	2
	//По кост-центру 2.07.ХХ	1
	//По кост-центру 2.11.02	1
    //По кост-центру 2.00.00	2
    //По кост-центру 2.00.01	2
	ТипЗатрат = Неопределено;
	Если Лев(СокрЛП(CostCentr.Наименование),5) = "2.05." Тогда
		ТипЗатрат = Перечисления.Pcru_ТипЗатрат.ТипЗатрат2;
	КонецЕсли;
	Если Лев(СокрЛП(CostCentr.Наименование),7) = "2.08.00" Тогда
		ТипЗатрат = Перечисления.Pcru_ТипЗатрат.ТипЗатрат2;
	КонецЕсли;
	Если Лев(СокрЛП(CostCentr.Наименование),7) = "2.11.01" Тогда
		ТипЗатрат = Перечисления.Pcru_ТипЗатрат.ТипЗатрат2;
	КонецЕсли;
	Если Лев(СокрЛП(CostCentr.Наименование),5) = "2.07." Тогда
		ТипЗатрат = Перечисления.Pcru_ТипЗатрат.ТипЗатрат1;
	КонецЕсли;
	Если Лев(СокрЛП(CostCentr.Наименование),7) = "2.11.02" Тогда
		ТипЗатрат = Перечисления.Pcru_ТипЗатрат.ТипЗатрат2;
	КонецЕсли;
	Если Лев(СокрЛП(СтатьяРасходов.Наименование),4) = "1395" Тогда
		ТипЗатрат = Перечисления.Pcru_ТипЗатрат.ТипЗатрат2;
		Если Лев(СокрЛП(CostCentr.Наименование),5) = "2.07." Тогда
			ТипЗатрат = Перечисления.Pcru_ТипЗатрат.ТипЗатрат1;
		КонецЕсли;
	КонецЕсли;
	Если Лев(СокрЛП(CostCentr.Наименование),7) = "2.00.00" Тогда
		ТипЗатрат = Перечисления.Pcru_ТипЗатрат.ТипЗатрат2;
	КонецЕсли;
	Если Лев(СокрЛП(CostCentr.Наименование),7) = "2.00.01" Тогда
		ТипЗатрат = Перечисления.Pcru_ТипЗатрат.ТипЗатрат2;
	КонецЕсли;
	возврат ТипЗатрат;
КонецФункции
...
&НаКлиенте
Процедура пкру_РаспределениеЗатратпкру_СтатьяРасходовПриИзменении(Элемент)
	ТекДанные = ЭтотОбъект.Элементы.пкру_РаспределениеЗатрат.ТекущиеДанные;
	ЭтотОбъект.Элементы.пкру_РаспределениеЗатрат.ТекущиеДанные.ТипЗатрат = пкру_РаспределениеЗатратпкру_СтатьяРасходовПриИзмененииНаСервере(ТекДанные.пкру_CostCentr,ТекДанные.пкру_СтатьяРасходов);
КонецПроцедуры

&НаСервере
Процедура ОтправитьВУМФОНаСервере()
	Pcru_WSWORKS.ОтправитьСчетВУМФО(ЭтотОбъект.Объект.Ссылка);	// Вставить содержимое обработчика.
КонецПроцедуры

&НаКлиенте
Процедура ОтправитьВУМФО(Команда)
	ОтправитьВУМФОНаСервере();
КонецПроцедуры
