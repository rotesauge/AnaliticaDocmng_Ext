...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоНовый ИЛИ (Параметры.Свойство("ЗаполнитьКакНовый") И Параметры.ЗаполнитьКакНовый) Тогда
		...
		Если НЕ ЗначениеЗаполнено(ВидДокумента) Тогда
			Объект.ВидДокумента=Справочники.аДокументооборотВидыДокументов.Внутренний;
			ВидДокумента=Объект.ВидДокумента;
		КонецЕсли;
...
Процедура УстановитьЗаголовокФайлы()
	Элементы.ФайлыПоручения.Заголовок=НСтр("en='Files (';ru='Файлы ('")+Формат(Объект.СсылкиНаФайлы.Количество(), "ЧН=; ЧГ=")+")";
...
Процедура НастроитьФормуПоВидуДокумента(ВидДокумента)
	...
	Элементы.ФайлыПоручения.Видимость=ВидДокумента.ИспользоватьФайлы;
	...
	Если Элементы.ФайлыПоручения.Видимость И ВидДокумента.НастройкаВидимостиМенюФайлыВключена И НЕ (ВидДокумента.НастройкаВидимостиМенюФайлыОтключитьНастройкуДляПолныхПрав И аДООбщееСерверПС.ЭтоРольПолныеПрава()) Тогда
...
Процедура ПриОткрытии(Отказ)
  ...
  ОткрытьФорму("Документ.аДокументооборотКорпоративныйДокумент.Форма.ФормаДокументаСогласование",ПараметрыФормы); 
   //Форма.ЗаполнитьСервер(); 
...
Процедура ПриОткрытииНаСервере(Отказ)
	...
	Если ЭтотОбъект.Объект.ВидДокумента.Наименование = "Список задач для Глав Отделов" Тогда
		Если ЕстьЗапущеныеБП(ЭтотОбъект.Объект.Ссылка) Тогда
			Этаформа.ТолькоПросмотр = Истина;
			Этаформа.КоманднаяПанель.ПодчиненныеЭлементы.ГруппаБизнесПроцессы.Доступность = Ложь;
		Иначе	
			Этаформа.ТолькоПросмотр = Ложь;
			Этаформа.КоманднаяПанель.ПодчиненныеЭлементы.ГруппаБизнесПроцессы.Доступность = Истина;
		КонецЕсли;
		
		Этаформа.Элементы.ПроизвольнаяТаблица.Видимость = Ложь;
		
		Этаформа.Элементы.ДеревоЗадач.Видимость = Истина;
...
Функция ЕстьЗапущеныеБП(ДоктСсылка)
	Запрос=Новый Запрос("ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	Запрос.УстановитьПараметр("Объект",ДоктСсылка);
...
Процедура СтраницыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	ИначеЕсли ТекущаяСтраница=Элементы.ФайлыПоручения Тогда
		УстановитьПредпросмотр();
...
Процедура УстановитьОбычныйРежимФормы()
	...
	Элементы.ФайлыПоручения.Видимость=ФлагВидимости;
...
Процедура УстановитьПростойРежимФормы()
	...
	Если Объект.СсылкиНаФайлы.Количество()=0 Тогда
		Элементы.ФайлыПоручения.Видимость=Ложь;
	...
	Если Элементы.ФайлыПоручения.Видимость
	...
	Если ПустаяСтрока(Объект.СодержаниеДокумента) 
		И НЕ ВидДокумента.ЗапретитьПустоеТекстовоеСодержимое
		И Элементы.ФайлыПоручения.Видимость
		Тогда
...
Процедура ПростойРежимДобавитьФайл(Команда)
	...
	Элементы.Страницы.ОтображениеСтраниц=ОтображениеСтраницФормы.ЗакладкиСнизу;
	Элементы.ФайлыПоручения.Видимость=Истина;
...
Процедура ВвестиНаОсновании(Команда)
	...
	Если Параметры.Ключ.Пустая() Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ru='Сначала запишите текущий документ.';en='Please write the current document.'"), 60, аОбщееКлиентПС.ЗаголовокДиалога());
		Возврат;
	...
	ОписаниеОповещения("Оповещение_ЗакрытиеФормыВвестиНаОсновании", ЭтотОбъект));		
...
Процедура КомандаТаблицуВДеревоНаСервере()
	Дерево = ТаблицаВДерево(РеквизитФормыВЗначение("Объект"), "ПроизвольнаяТаблица");//Товары - имя табличной части
	ЗначениеВРеквизитФормы(Дерево, "ДеревоЗадач");//ДеревоЗначений - реквизит формы с типом дерево значений
	Элементы.ДеревоЗадач.Обновить();
КонецПроцедуры


&НаСервере
Процедура КомандаДеревоВТаблицуНаСервере()
	Объект.ПроизвольнаяТаблица.Очистить();
	ДОбъект = РеквизитФормыВЗначение("Объект");
	ДеревоВТаблицу(ДОбъект, РеквизитФормыВЗначение("ДеревоЗадач"), "ПроизвольнаяТаблица");
...
Функция ТаблицаВДерево(ДокОбъект, НаименованиеТабличнойЧастиДокумента, КлючСвязи = NULL, ЭлементРодитель = NULL) Экспорт	
	...
	ДеревоЗначений2 = РеквизитформыВЗначение("ДеревоЗадач");
	
	ДеревоЗначений2.Строки.Очистить();
	...
	Фильтр = Новый Структура("КолонкаЧисло1", КлючСвязи);
	М = ДокОбъект[НаименованиеТабличнойЧастиДокумента].НайтиСтроки(Фильтр);
	 ...
	 Запрос.Текст = "ВЫБРАТЬ
	                ...
	                |	И pcru_ПорученияГлавамОтделов.КлючСвязи = &КлючСвязи";
	...
	КонецЕсли;
	 ...
	 Пока Выборка.Следующий() Цикл
		...
		Элемент.Задача = Выборка.Задача;
		Элемент.Отдел  = Выборка.Отдел;
		Элемент.Результат = Выборка.Результат;
		Элемент.Исполнитель = Выборка.Исполнитель;
		Элемент.СрокВыполнения = Выборка.СрокВыполнения;
    	Элемент.Принято = Выборка.Принято;
		...
		ТаблицаВДерево(ДокОбъект, НаименованиеТабличнойЧастиДокумента, Выборка.НомерСтрокиДокумента, Элемент); //ДОБАВЛЕНИЕ ПОДЧИНЁННЫХ ЭЛЕМЕНТОВ В ДЕРЕВО
...
Процедура ДеревоВТаблицу(ДокОбъект, ДеревоЗначений, НаименованиеТабличнойЧастиДокумента, СтрокаДерева = NULL, КлючСвязи = NULL) Экспорт
	...
	Если СтрокаДерева = NULL И КлючСвязи = NULL Тогда
		//ПЕРВЫЙ ВЫЗОВ ПРОЦЕДУРЫ (КОРНЕВЫЕ ЭЛЕМЕНТЫ)
    	РегистрыСведений.pcru_ПорученияГлавамОтделов.ОчиститьПоДокументу(ДокОбъект);
		ПервыйВызов = Истина;
	...
	Для каждого Стр из ИсточникВыборки Цикл
		...
		МЗ.Документ = ДокОбъект.Ссылка;
        МЗ.КлючСвязи = КлючСвязи;
        МЗ.НомерСтрокиДокумента = РегистрыСведений.pcru_ПорученияГлавамОтделов.ПолучитьМаксимальныйНомерСтроки(ДокОбъект);
		
		МЗ.Задача = Стр.Задача;
		МЗ.Отдел = Стр.Отдел;
		МЗ.Результат = Стр.Результат;
		МЗ.Исполнитель = Стр.Исполнитель;
		МЗ.СрокВыполнения   = Стр.СрокВыполнения;
		МЗ.Принято   = Стр.Принято;
		МЗ.Записать(Истина);
		//НС = ДокОбъект[НаименованиеТабличнойЧастиДокумента].Добавить();
		//НС.КолонкаЧисло1 = КлючСвязи;
		//
		//НС.КолонкаСтрока1 = Стр.Задача;
		//НС.КолонкаСсылка1 = Стр.Отдел;
		//НС.КолонкаСтрока2 = Стр.Результат;
		//НС.КолонкаСсылка2 = Стр.Исполнитель;
		//НС.КолонкаДата1   = Стр.СрокВыполнения;
		...
		Если НЕ Стр.Строки.Количество() = 0 Тогда
			ДеревоВТаблицу(ДокОбъект,,НаименованиеТабличнойЧастиДокумента, Стр, МЗ.НомерСтрокиДокумента);
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры 
...
Процедура ДеревоЗадачПриИзменении(Элемент)
	Для Каждого Стр из ДеревоЗадач.ПолучитьЭлементы() Цикл
...
Процедура ОбойтиДеревоЗначений(Элемент)
	ТекОтдел                      = Элемент.Отдел;
	ТекСрокВыполнения             = Элемент.СрокВыполнения;
	ПодчиненныйЭлемент            = Элемент.ПолучитьЭлементы();
	Для Каждого Строка из ПодчиненныйЭлемент Цикл
		Если Не ЗначениеЗаполнено(Строка.Отдел) Тогда
			Строка.Отдел          = ТекОтдел;
			Строка.СрокВыполнения = ТекСрокВыполнения;
		КонецЕсли;   
...
&НаСервереБезКонтекста
Процедура ДеревоЗадачПередНачаломДобавленияНаСервере()
...
Процедура ДеревоЗадачПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	ДеревоЗадачПередНачаломДобавленияНаСервере();
КонецПроцедуры

&НаСервереБезКонтекста
Процедура ДеревоЗадачПриОкончанииРедактированияНаСервере()
	// Вставить содержимое обработчика.
КонецПроцедуры

&НаКлиенте
Процедура ДеревоЗадачПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	ДеревоЗадачПриОкончанииРедактированияНаСервере();
КонецПроцедуры

&НаСервере
Процедура ДеревоЗадачДобавитьКорневойНаСервере()
	НовыйЭлемент = ЭтаФорма.ДеревоЗадач.ПолучитьЭлементы().Добавить();
	// Вставить содержимое обработчика.
КонецПроцедуры

&НаКлиенте
Процедура ДеревоЗадачДобавитьКорневой(Команда)
	ДеревоЗадачДобавитьКорневойНаСервере();
КонецПроцедуры

&НаСервере
Процедура ДеревоЗадачДобавитьПодзадачуНаСервере()
КонецПроцедуры

&НаКлиенте
Процедура ДеревоЗадачДобавитьПодзадачу(Команда)
	ТекущиеДанные = Элементы.ДеревоЗадач.ТекущиеДанные;
	НовыйЭлемент =  ТекущиеДанные.ПолучитьЭлементы().Добавить();
	ОбойтиДеревоЗначений(ТекущиеДанные);
	Элементы.ДеревоЗадач.Развернуть(Элементы.ДеревоЗадач.ТекущаяСтрока);
КонецПроцедуры
